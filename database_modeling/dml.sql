...
(SELECT
	s_r.rental_id,
	s_r.last_update                                          AS rental_last_update,
	d_c.customer_key,
	d_sf.staff_key,
	d_f.film_key,
	d_sr.store_key,
	d_dre.date_id                                            AS rental_date_key,
	d_drn.date_id                                            AS return_date_key,
    CASE WHEN d_drn.date_id IS NOT NULL THEN 1 ELSE 0 END    AS count_returns,
    CASE WHEN d_dre.date_id IS NOT NULL THEN 1 ELSE 0 END    AS count_rentals,
	d_f.film_rental_duration                                 AS rental_duration,
	s_p.amount                                               AS dollar_amount
FROM
	sakila.rental s_r
	LEFT JOIN sakila.inventory s_i                  ON s_i.inventory_id = s_r.inventory_id
	LEFT JOIN sakila_snowflake.dim_customer d_c     ON d_c.customer_id  = s_r.customer_id
	LEFT JOIN sakila_snowflake.dim_staff d_sf       ON d_sf.staff_id    = s_r.staff_id
	LEFT JOIN sakila_snowflake.dim_film d_f         ON d_f.film_id      = s_i.film_id
	LEFT JOIN sakila_snowflake.dim_store d_sr       ON d_sr.store_id    = s_i.store_id    
	LEFT JOIN sakila_snowflake.dim_date d_dre       ON DATE(d_dre.date) = DATE(s_r.rental_date)
	LEFT JOIN sakila_snowflake.dim_date d_drn       ON DATE(d_drn.date) = DATE(s_r.return_date)
	LEFT JOIN sakila.payment s_p                    ON s_p.rental_id    = s_r.rental_id
	);